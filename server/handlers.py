...
def handle_default_request(bytes_request):
    ...
    if validate_request(request):
        ...
        if controller:
            try:
                logging.info(f'Clients request valid {request}')
                response = controller(request)
            except Exception as err:
                logging.critical(f'Internal server error: {err}')
                response = make_responce(request, 500, data='Internal server error')
        else:
            logging.error(f'Unexisted controller: {action_name} ')
